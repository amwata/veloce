...
const UX = function () {
		...
		.then(res => {
			...
			for(let i = 0; i < arrvls.length; i++){
				...
				box.innerHTML = `<div class="arv-desc">	
								 	...
								 	<p>USD. ${this.zip(arrvls[i][6].toString())}</p>
								</div>
		...
		for(let i = 0; i < len; i++){
			...
			box.innerHTML = `<div class="item-box">
								...
								<div class="item-desc">	
									...
									<h4>USD. ${this.zip(items[i][6].toString())}</h4>
								</div>
							</div>`
			ctn.appendChild(box)
		}
	...
	Client_UX.prototype.loadCheckOuts = function (el){
		...
		try{
			...
			for(let [k, v] of Object.entries(cart)){
				...
				<td class=""><h3>USD. ${match[0][6]*v}</h3></td>
				</tr>`
	...
	Client_UX.prototype.loadOrder = function (el){
		...
		try{
			...
			<td><h3>USD. ${total}</h3></td>
			</tr>
			<tr>
			<td colspan="2"><h3>Delivery fee</h3></td>
			<td><h3>USD. ${this.zip(delFee.toFixed(2).toString())}</h3></td>
			</tr>
			<tr>
			<td colspan="2">TOTAL</td>
			<td>USD. ${this.zip(Ttl.toString())}</td>
			</tr>`
			...
			for(const [k,v] of Object.entries(jsn)){
				...
				for(const [x, y] of Object.entries(items)){
					...
					<tr>
						...
						<td><h3>${this.zip(match[0][1]+" "+match[0][3])}</h3><p>USD. ${this.zip(match[0][6].toString())}.00</p></td>
						<td>USD. ${this.zip(ttl.toString())}.00</td>	
					</tr>`
				}
				...
				foldTab.innerHTML += `
					<tr class="top-hr">
						...
						<td>USD. ${this.zip(amt.toString())}.00</td>
					</tr>
					<tr>
						<td colspan="2">Delivery fee</td>
						<td>USD. ${this.zip(delFee.toFixed(2).toString())}</td>
					</tr>
					<tr class="lc">
						<td colspan="2">TOTAL</td>
						<td>USD. ${this.zip(Ttl.toString())}</td>
	...
	Client_UX.prototype.loadCart = function(el){
		...
		try{
			...
			for(let [k, v] of Object.entries(cart)){
				...
				<td><h3>${this.zip(match[0][1]+" "+match[0][3])}</h3><p>USD. ${this.zip(match[0][6].toString())}</p></td>
				<td>
				...
				<td class="cart-dsk-price"><h3>USD. ${Number(match[0][6])*v}</h3></td>
				</tr>`
				total += Number(match[0][6])*v
			}tbody.innerHTML += `<tr>
			<td>TOTAL</td>
			...
			ctn1.innerHTML = `<div class="maxwid shop">
				...
				<div class="shop-detail-action">
					<div class="shop-detail">
						...
						<p>USD. ${this.zip(item[6].toString())}</p>
					</div>
				...
				for(let i = len - 1; i >= 0; i--){
					...
					box.innerHTML = `<div class="item-box">
							...
							<div class="item-desc">	
								...
								<h4>USD. ${this.zip(other[i][6].toString())}</h4>
							</div>
						</div>`
				...
				for(let i = len - 1; i >= 0; i--){
					...
					ctni.innerHTML = `
										...
										<div class="srch-result-desc">	
											...
											<p>USD. ${this.zip(drop[i][6].toString())}</p>
										</div>`
